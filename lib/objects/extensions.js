...
const filename = 'extensions-backup.json';
 ...
 */
async function getExtensions(jwt, url) {
  return portainer.getObjects(jwt, url, portainer.EXTENSIONS);
}
...
async function getExtension(jwt, url, id) {
  return portainer.getObjectDetail(jwt, url, id, portainer.EXTENSIONS);
}
...
async function backup(jwt, url) {
  ...
  const extensions = await getExtensions(jwt, url);
  ...
  for (let i = 0; i < extensions.length; i += 1) {
    ...
    extensionPromisses.push(
      getExtension(jwt, url, extension.Id).then((extensionData) => {
