...
test('Log user', async (t) => {
  const jwt = await portainer.login(config.portainer.url, config.portainer.login,
...
test('Get all stacks', async (t) => {
  const data = await portainer.getStacks(undefined, config.portainer.url);
...
test('Get stack file', async (t) => {
  const data = await portainer.getStackFile(undefined, config.portainer.url, 'id');
